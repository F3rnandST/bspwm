...
apply_dunst() {
    ...
    -e "s/frame_color = .*/frame_color = $dunst_border_color/g" \
		-e "s/separator_height = .*/separator_height = $dunst_separator/g" \
		-e "s/line_height = .*/line_height = $dunst_separator/g"

	# modify colors
	sed -i '/urgency_low/Q' ${PATH_CONF}/dunst/dunstrc
	cat >> ${PATH_CONF}/dunst/dunstrc <<- _EOF_
		[urgency_low]
		timeout = 2
		background = "${background}"
		foreground = "${foreground}"
...
apply_theme() {
 sh ~/.config/bspwm/themes/night/theme & 
}
