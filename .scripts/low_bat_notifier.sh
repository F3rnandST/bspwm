...
ASSETS_DIR="$HOME/.icons"

### VARIABLES
POLL_INTERVAL=60
LOW_BAT=25
FULL_BAT=90
play mpv --mute=no /usr/share/sounds/freedesktop/stereo/complete.oga
...
then 
    ...
    do
        ...
        if [[ $bat_percent -lt $LOW_BAT && "$bs" = "Discharging" && $launched -lt 12 ]]
        then
            notify-send --urgency=critical "$bat_percent% > Low Battery!" -i "$ASSETS_DIR/low-battery.png"
            launched=$((launched+1))
        elif [[ "$bs" = "Charging" ]]
        then
            launched=0
...
#mpv --mute=no /usr/share/sounds/freedesktop/stereo/complete.oga
